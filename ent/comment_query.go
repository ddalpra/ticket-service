...
func (cq *CommentQuery) QueryTicket() *TicketQuery {
	query := (&TicketClient{config: cq.config}).Query()
...
func (cq *CommentQuery) WithTicket(opts ...func(*TicketQuery)) *CommentQuery {
	query := (&TicketClient{config: cq.config}).Query()
...
//		GroupBy(comment.FieldOrderComment).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Comment.Query().
//		Select(comment.FieldOrderComment).
//		Scan(ctx, &v)
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if cq.withTicket != nil || cq.withAuthor != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withTicket; query != nil {
		if err := cq.loadTicket(ctx, query, nodes, nil,
			func(n *Comment, e *Ticket) { n.Edges.Ticket = e }); err != nil {
			return nil, err
		}
	}
...
func (cq *CommentQuery) loadTicket(ctx context.Context, query *TicketQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Ticket)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Comment)
	for i := range nodes {
		if nodes[i].ticket_comments == nil {
			continue
		}
		fk := *nodes[i].ticket_comments
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ticket_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CommentQuery) loadAttachments(ctx context.Context, query *AttachmentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Attachment)) error {
	...
	query.Where(predicate.Attachment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(comment.AttachmentsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.comment_attachments
		if fk == nil {
			return fmt.Errorf(`foreign-key "comment_attachments" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "comment_attachments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
