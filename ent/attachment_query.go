...
func (aq *AttachmentQuery) QueryTicket() *TicketQuery {
...
func (aq *AttachmentQuery) QueryComment() *CommentQuery {
...
func (aq *AttachmentQuery) QueryUploadedBy() *UserQuery {
...
func (aq *AttachmentQuery) Clone() *AttachmentQuery {
	...
	return &AttachmentQuery{
		...
		withUploadedBy: aq.withUploadedBy.Clone(),
...
func (aq *AttachmentQuery) WithTicket(opts ...func(*TicketQuery)) *AttachmentQuery {
	query := (&TicketClient{config: aq.config}).Query()
...
func (aq *AttachmentQuery) WithComment(opts ...func(*CommentQuery)) *AttachmentQuery {
	query := (&CommentClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AttachmentQuery) WithUploadedBy(opts ...func(*UserQuery)) *AttachmentQuery {
	query := (&UserClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withUploadedBy = query
	return aq
}
...
func (aq *AttachmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attachment, error) {
	var (
		...
		loadedTypes = [3]bool{
			aq.withTicket != nil,
			aq.withComment != nil,
			aq.withUploadedBy != nil,
		}
	)
	if aq.withTicket != nil || aq.withComment != nil || aq.withUploadedBy != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withTicket; query != nil {
		if err := aq.loadTicket(ctx, query, nodes, nil,
			func(n *Attachment, e *Ticket) { n.Edges.Ticket = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withComment; query != nil {
		if err := aq.loadComment(ctx, query, nodes, nil,
			func(n *Attachment, e *Comment) { n.Edges.Comment = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withUploadedBy; query != nil {
		if err := aq.loadUploadedBy(ctx, query, nodes, nil,
			func(n *Attachment, e *User) { n.Edges.UploadedBy = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttachmentQuery) loadTicket(ctx context.Context, query *TicketQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Ticket)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "ticket_attachments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AttachmentQuery) loadComment(ctx context.Context, query *CommentQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *Comment)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attachment)
	for i := range nodes {
		if nodes[i].comment_attachments == nil {
			continue
		}
		fk := *nodes[i].comment_attachments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "comment_attachments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AttachmentQuery) loadUploadedBy(ctx context.Context, query *UserQuery, nodes []*Attachment, init func(*Attachment), assign func(*Attachment, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Attachment)
	for i := range nodes {
