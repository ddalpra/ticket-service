...
// The ServiceCenterFunc type is an adapter to allow the use of ordinary
// function as ServiceCenter mutator.
type ServiceCenterFunc func(context.Context, *ent.ServiceCenterMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ServiceCenterFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ServiceCenterMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ServiceCenterMutation", m)
}
