...
func (cq *CompanyQuery) QueryTickets() *TicketQuery {
...
func (cq *CompanyQuery) Clone() *CompanyQuery {
	...
	return &CompanyQuery{
		...
		withTickets: cq.withTickets.Clone(),
...
func (cq *CompanyQuery) WithTickets(opts ...func(*TicketQuery)) *CompanyQuery {
	query := (&TicketClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withTickets = query
	return cq
}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Company, error) {
	...
	if query := cq.withTickets; query != nil {
		if err := cq.loadTickets(ctx, query, nodes,
			func(n *Company) { n.Edges.Tickets = []*Ticket{} },
			func(n *Company, e *Ticket) { n.Edges.Tickets = append(n.Edges.Tickets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CompanyQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*Company, init func(*Company), assign func(*Company, *Ticket)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Company)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Ticket(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(company.TicketsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.company_tickets
		if fk == nil {
			return fmt.Errorf(`foreign-key "company_tickets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "company_tickets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
